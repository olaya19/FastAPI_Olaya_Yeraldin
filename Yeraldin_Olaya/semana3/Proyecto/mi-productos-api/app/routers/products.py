...
def update_product(product_id: int, product_update: product_schemas.ProductCreate):
    ...
    for existing_id, existing_product in products_db.items():
        if (
            ...
            and existing_product.name.lower() == product_update.name.lower()
        ):
            validation_error(
                f"Ya existe otro producto con el nombre '{product_update.name}'"
            )
...
def update_product_partial(
    product_id: int, product_update: product_schemas.ProductUpdate
):
