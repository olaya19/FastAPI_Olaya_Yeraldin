...
def create_first_admin(user_data: UserRegister, db: Session = Depends(get_db)):
    ...
    if existing_admin:
        raise HTTPException(
            status_code=400, detail="Ya existe un administrador en el sistema"
        )
...
def list_all_users(
    admin_user: User = Depends(auth.require_admin), db: Session = Depends(get_db)
):
...
@app.post("/posts")
def create_post(post: Post, current_user=Depends(get_current_user)):  # Requiere login
    new_post = {
        "id": len(posts) + 1,
        "title": post.title,
